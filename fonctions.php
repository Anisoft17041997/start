...
function modif_pass($email,$pass)
{
  require('config/connect.php');
  $req = $bdd->prepare('UPDATE membres SET pass=:pass WHERE email=:email');
  $req->execute(array('pass'=>sha1($pass), 'email'=>$email));
  $req->closeCursor();
}
...
{
  require('config/connect.php');
  $req = $bdd->prepare('SELECT * FROM topics ORDER BY id ASC');
  $req->execute();
  $data = $req->fetchAll();
  return $data;
  $req->closeCursor();
