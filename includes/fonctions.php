...
if(!function_exists('afficher_news')){
    function afficher_news(){
        ...
        //nombre de news
        $q = $db->prepare("SELECT count(*) nombre FROM news");
        $q->execute(array());
        $nb_news = $q->fetch();

        return array('news' =>  $news , 'nb_news' => $nb_news);
    }
}
...
//fonction permettant de supprimer un new
if(!function_exists('supprimer_new')){
    function supprimer_new($key)
    {
        global $db;
...
if(!function_exists('dec_nb_kit')){
    function dec_nb_kit($key, $nb_kit_courant)
    {
        ...
        $query = $db->prepare("
                ...
                WHERE id = ?
                ");
        $temp = ($nb_kit_courant-1 <= 0) ? 0 : $nb_kit_courant-1;
        $query->execute(array($temp, $key));
        header('Location: admin.php');
    }
}
